...
TARGETS = [
    {
        "config": ROOT_DIR / "spyre_inference/config/model_configs.yaml",
        "doc": ROOT_DIR / "docs/user_guide/supported_models.md",
    },
    {
        "config": ROOT_DIR / "docs/legacy/model_configs.yaml",
        "doc": ROOT_DIR / "docs/legacy/user_guide/supported_models.md",
    },
]
...
def generate_tables(config_path: Path):
...
def update_supported_models_doc(generative_table, pooling_table, doc_path: Path):
...
def on_startup(command: Literal["build", "gh-deploy", "serve"], dirty: bool):
    ...
    for target in TARGETS:
        ...
        if not config_path.exists() or not doc_path.exists():
            print(f"Skipping model table generation: {config_path} or {doc_path} not found")
            continue

        print(f"Generating model configuration tables from {config_path}")
        generative_table, pooling_table = generate_tables(config_path)
        ...
        update_supported_models_doc(generative_table, pooling_table, doc_path)
