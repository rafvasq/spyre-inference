...
LEGACY_EXAMPLE_DIR = ROOT_DIR / "docs/legacy/examples"
...
def process_examples(example_dir: Path, doc_dir: Path, include_source_link: bool = True):
    if not example_dir.exists():
        print(f"Skipping example generation: {example_dir} not found")
        return
    ...
    for category in categories:
        ...
        for path in itertools.chain(*globs):
            examples.append(Example(path, category.stem, include_source_link))
        for path in category.glob("*/*.md"):
            examples.append(Example(path.parent, category.stem, include_source_link))

    for example in sorted(examples, key=lambda e: e.path.stem):
        example_name = f"{example.path.stem}.md"
        doc_path = doc_dir / example.category / example_name
        print(doc_path)
...
def on_startup(command: Literal["build", "gh-deploy", "serve"], dirty: bool):
    process_examples(EXAMPLE_DIR, EXAMPLE_DOC_DIR, include_source_link=True)
    process_examples(LEGACY_EXAMPLE_DIR, LEGACY_EXAMPLE_DIR, include_source_link=False)
